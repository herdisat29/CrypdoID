...
export default function ScamAnalyzer() {
  ...
  const [hybridAnalysis, setHybridAnalysis] = useState<AnalysisResult | null>(null);
...
Gunakan data ini sebagai referensi faktual. Bila ada inkonsistensi antara data dan klaim project, tandai sebagai red flag.`
    ...
    const prompt = `Kamu adalah Scam Radar dari CrypdoID. Jawab HANYA dalam format JSON valid.

Project: ${input}
${marketContext}

INSTRUKSI KHUSUS:
- Jika token ini resmi dari project besar (seperti Manta Network, Arbitrum, Optimism, dll), jangan terlalu paranoid. Beri score yang realistis.
- Bedakan antara "token resmi L2" vs "memecoin random".
- Prioritaskan fakta on-chain daripada rumor.
- Risk Score 0-100 (semakin tinggi = semakin berbahaya)
...
Kembalikan JSON:
{
  ...
  "takeaway": "<string>"
    ...
    try {
      ...
      profile.addScannedToken(input.trim());
  ...
  const runGeminiAnalysisForHybrid = async (query: string): Promise<AnalysisResult | null> => {
    try {
      const cgData = await fetchCoinGeckoData(query);
      const marketContext = cgData
        ? `DATA PASAR REAL (CoinGecko):\n- Nama: ${cgData.name} (${cgData.symbol})\n- Market Cap: ${cgData.marketCap}\n- Volume 24h: ${cgData.volume24h}\n- Perubahan Harga: 24h ${cgData.priceChange24h}% | 7d ${cgData.priceChange7d}%\n- Kategori: ${cgData.categories}`
        : `[Data CoinGecko tidak tersedia]`;
...
Kembalikan JSON:
{
  ...
  "takeaway": ""
      ...
      return { raw: response, riskScore: score, parsed };
    } catch {
      return null;
    }
  };

  // Run On-chain smart contract analysis + Gemini Hybrid
  const handleOnChainScan = async () => {
    ...
    setFinalRiskScore(null);
    setAutoSwitchNotice('');

    try {
      const cacheRef = doc(db, 'tokenScans', contractAddress.trim().toLowerCase());
      const cacheSnap = await getDoc(cacheRef);
      if (cacheSnap.exists()) {
        const data = cacheSnap.data();
        if (data.lastScanned && Date.now() - data.lastScanned < 24 * 60 * 60 * 1000) {
          setSecurityResult(data.onchainData);
          setHybridAnalysis(data.geminiData);
          setFinalRiskScore(data.finalScore);
          
          if (data.onchainData.detectedChainId && data.onchainData.detectedChainId !== selectedChain) {
            setSelectedChain(data.onchainData.detectedChainId);
          }
      ...
      // Step 2: AI Gemini Scan
      let geminiRes = null;
      let geminiScore = 50; // default if not found
      if (result.tokenName && result.tokenName !== 'Unknown Token') {
         geminiRes = await runGeminiAnalysisForHybrid(result.tokenName);
         if (geminiRes) {
           setHybridAnalysis(geminiRes);
           geminiScore = geminiRes.riskScore;
         }
      }

      // Step 3: Hybrid Scoring
      const onchainScore = result.riskScore;
      const finalScore = Math.round((geminiScore * 0.45) + (onchainScore * 0.55));
      ...
      try {
        await setDoc(cacheRef, {
          ...
          chainId: detectedId || selectedChain,
          lastScanned: Date.now(),
          geminiScore: geminiScore,
          onchainScore: onchainScore,
          finalScore: finalScore,
          onchainData: result,
          geminiData: geminiRes
        });
      } catch (e) {
      ...
      const profile = useUserStore.getState();
      profile.setProfileFlag('scanCount', profile.scanCount + 1);
      profile.addScannedToken(contractAddress.trim());

      // Mission 7: hanya trigger kalau isHoneypot === true
  ...
  const chainList = [
    { id: 900, name: 'Solana', short: 'SOL', logo: '🟣', color: 'text-purple-400', bg: 'bg-purple-500/10' },
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  type="text" 
                  placeholder="CONTRACT ADDRESS (0x... atau base58)" 
              ...
              <p className="text-[10px] text-emerald-500/70 italic mt-2 font-bold px-1">
                *Smart Scan akan otomatis kombinasikan On-Chain + AI Analysis
              </p>
            </div>
            ...
            <AnimatePresence mode="wait">
              ...
              {securityResult && !isScanningContract && (
                ...
                >
                  {/* Hybrid 3-Column Header */}
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 border-b border-white/10 pb-8 items-stretch pt-4">
                    <h2 className="text-3xl md:text-4xl font-black text-white tracking-tighter col-span-1 md:col-span-3 text-center mb-2 md:mb-4">
                      {securityResult.tokenName} ({securityResult.tokenSymbol})
                      {securityResult.isSimulated && (
                        <span className="ml-3 text-xs font-black bg-yellow-500/20 text-yellow-400 border border-yellow-500/40 px-2.5 py-1.5 rounded-lg align-middle tracking-widest uppercase not-italic animate-pulse">
                          SIMULATED
                        </span>
                      )}
                      <p className="uppercase text-[10px] md:text-xs tracking-widest font-black mt-2 text-slate-500 block">
                        {securityResult.chain} NETWORK
                      </p>
                    </h2>
                    
                    {/* Column 1: AI Sentiment */}
                    <div className="text-center p-4 md:p-6 bg-black/20 rounded-3xl border border-white/5 h-full flex flex-col justify-center">
                       <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">AI Sentiment</h3>
                       {hybridAnalysis ? (
                         <>
                           <div className={`text-5xl font-black mb-2 ${getRiskTextColor(hybridAnalysis.parsed?.verdict === 'BAHAYA' ? 'HIGH' : hybridAnalysis.parsed?.verdict === 'HATI-HATI' ? 'MEDIUM' : 'LOW')}`}>
                             {hybridAnalysis.riskScore}
                           </div>
                           <p className="text-xs font-bold text-slate-300 uppercase tracking-widest">{hybridAnalysis.parsed?.verdict || 'ANALYZED'}</p>
                         </>
                       ) : (
                         <div className="text-xs text-slate-500 font-bold italic py-4">No AI Context</div>
                       )}
                    </div>

                    {/* Column 2: On-Chain Scan */}
                    <div className="text-center p-4 md:p-6 bg-black/20 rounded-3xl border border-white/5 h-full flex flex-col justify-center">
                       <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-3">On-Chain Scan</h3>
                       <div className={`text-5xl font-black mb-2 ${getRiskTextColor(securityResult.overallRisk)}`}>
                         {securityResult.riskScore}
                       </div>
                       <p className="text-xs font-bold text-slate-300 uppercase tracking-widest">{securityResult.overallRisk}</p>
                    </div>

                    {/* Column 3: Final Risk */}
                    <div className="text-center p-6 md:p-8 bg-vibrant-purple/10 rounded-3xl border border-vibrant-purple/30 shadow-[0_0_40px_rgba(126,34,206,0.15)] md:scale-110 z-10 h-full flex flex-col justify-center relative mt-4 md:mt-0">
                       <h3 className="text-xs font-black uppercase tracking-widest text-gold-accent mb-3">Final Risk Score</h3>
                       <div className={`text-7xl font-black mb-4 ${getRiskTextColor(finalRiskScore !== null ? (finalRiskScore >= 70 ? 'HIGH' : finalRiskScore >= 30 ? 'MEDIUM' : 'LOW') : securityResult.overallRisk)}`}>
                         {finalRiskScore ?? securityResult.riskScore}
                       </div>
                       <p className="text-[10px] font-black text-white uppercase tracking-widest bg-black/60 py-1.5 px-4 rounded-full mx-auto inline-block border border-white/10">
                         {finalRiskScore !== null ? (finalRiskScore >= 70 ? 'HIGH RISK' : finalRiskScore >= 30 ? 'CAUTION' : 'SAFE') : securityResult.overallRisk}
                       </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 relative z-10 text-xs text-slate-300 font-bold">
                    ...
                    <div className="p-4 bg-black/20 rounded-xl border border-white/5 flex items-center justify-between md:col-span-2">
                      <span className="text-slate-400">Source Code:</span>
                      <span className={securityResult.isVerified ? "text-emerald-400 font-black flex items-center gap-1" : "text-red-400 font-black flex items-center gap-1"}>
                        {securityResult.isVerified ? (
                          <>✅ Verified ({securityResult.verificationSource})</>
                        ) : (
                          <>🚨 Unverified (Tertutup)</>
                        )}
                      </span>
